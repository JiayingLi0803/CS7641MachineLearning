...
def complete_(data): # [1pts]
    ...
    for i in range(len(data)):
        if True not in np.isnan(data)[i]:
            complete_rows.append(data[i])
    return np.array(complete_rows)
...
def incomplete_(data): # [1pts]
    ...
    incomplete_rows = []
    for i in range(len(data)):
        if True in np.isnan(data)[i] and np.isnan(data)[i][-1] != True:
            incomplete_rows.append(data[i])
    return np.array(incomplete_rows) 
    # raise NotImplementedError
...
def unlabeled_(data): # [1pts]
    ...
    unlabeled_rows = []
    for i in range(len(data)):
        if True not in np.isnan(data)[:,:-1][i] and np.isnan(data)[i][-1] == True:
            unlabeled_rows.append(data[i])
    return np.array(unlabeled_rows)
    # raise NotImplementedError
...
class CleanData(object):
    ...
    def __call__(self, incomplete_points,  complete_points, K, **kwargs): # [7pts]
        ...
        """
        for i in range(len(incomplete_points)):
            nanindex = int(np.argwhere(np.isnan(incomplete_points[i])))
            cutcom = complete_points[np.where(incomplete_points[:,-1] == incomplete_points[i][-1])]
            newcom = np.delete(cutcom, [nanindex,-1], axis=1) 
            newincom = np.delete(incomplete_points, [nanindex,-1], axis=1)[i]
            #print("newcom: ", newcom)
            #print("newincom: ", newincom)
            dist_matrix = self.pairwise_dist(np.array([newincom]), newcom)
            #print("dist_matrix: ", dist_matrix)
            order = np.argsort(dist_matrix)[0]
            #print("order: ", order)
            knrows = cutcom[order[:K]]
            #print("knrows: ", knrows)
            #print("mean values: ", np.mean(knrows, axis = 0))
            incomplete_points[i][nanindex] = np.mean(knrows, axis = 0)[nanindex]
            #print("new points: ", incomplete_points)
        #print("final_values: ", np.concatenate((incomplete_points, complete_points), axis=0))
        return np.concatenate((incomplete_points, complete_points), axis=0)
...
def mean_clean_data(data): # [2pts]
    ...
    """
    meanrow = np.round_((np.nanmean(data,axis = 0)),decimals=1)
    for i in range(len(data)):
        if True in np.isnan(data[i]):
            nanindex = int(np.argwhere(np.isnan(data[i])))
            data[i][nanindex] = meanrow[nanindex]
    #print(data)
    return data
    # raise NotImplementedError
