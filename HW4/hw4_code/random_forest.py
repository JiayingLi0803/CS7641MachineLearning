...
class RandomForest(object):
    ...
    def _bootstrapping(self, num_training, num_features, random_seed = None):
        ...
        a = np.random.choice(num_training, num_training, replace=True)
        #################################################################################
        b = np.random.choice(num_features, int(num_features*self.max_features), replace=False)
    ...
    def fit(self, X, y):
        ...
        for i in range(self.n_estimators):
            a = self.bootstraps_row_indices[i]
            b = self.feature_indices[i]
            #print(a, b)
            xtrain =  X[a][:, b]
            ytrain = y[a]
    ...
    def plot_feature_importance(self, data_train):
        ...
        feature_names = np.array(["cap-shape", "cap-surface", "cap-color", "bruises", "odor", "gill-attachment", "gill-spacing", 
                        "gill-size", "gill-color", "stalk-shape", "stalk-root", "stalk-surface-above-ring", 
                        "stalk-surface-below-ring", "stalk-color-above-ring", "stalk-color-below-ring", "veil-type", 
                        "veil-color", "ring-number", "ring-type", "spore-print-color", "population", "habitat"])
        for i in range(self.n_estimators):
            # print(self.decision_trees[i].feature_importances_)
            xname = feature_names[self.feature_indices[i]]
            plt.figure(figsize=(15,6))
            importance_val = self.decision_trees[i].feature_importances_
            sortidx = np.argsort(importance_val)[::-1]
            sortxname = xname[sortidx]
            sortval = importance_val[sortidx]
            plt.bar(sortxname, sortval)
