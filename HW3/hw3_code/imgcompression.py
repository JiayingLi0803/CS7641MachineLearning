...
class ImgCompression(object):
    ...
    def svd(self, X: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:  # [4pts]
        ...
        if len(X.shape) == 2:
            ...
            for i in range(3):
                ...
                slist.append(s.T)
                vtlist.append(vt.T)
            return (np.array(ulist).T,np.array(slist).T,np.array(vtlist).T)
    ...
    def rebuild_svd(
        ...
        if len(U_compressed.shape) == 2:
            return np.matmul((U_compressed*S_compressed),V_compressed)
        else:
            Xrebuild = np.zeros((U_compressed.shape[0], V_compressed.shape[1], 3))
            for i in range(3):
                Xrebuild[:,:,i] = np.matmul((U_compressed[:,:,i]*S_compressed[:,i]),V_compressed[:,:,i])
            return np.array(Xrebuild)
        # raise NotImplementedError
    ...
    def compression_ratio(self, X: np.ndarray, k: int) -> float:  # [4pts]
        ...
        """
        uncompressedSize = X.shape[0]*X.shape[1]
        compressedSize = X.shape[1]*k + k + k*X.shape[0]
    ...
    def recovered_variance_proportion(self, S: np.ndarray, k: int) -> float:  # [4pts]
        ...
        if len(S.shape) == 1:
            ...
            return np.sum(S[:k]**2,axis = 0)/np.sum(S**2,axis = 0)
        # raise NotImplementedError
